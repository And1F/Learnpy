...
def select_range(data):
    ...
    if rnge == "":pass
    elif rnge.isnumeric() and 0 < int(rnge) < len(data): 
        print(f"The last you skipped was {data[-int(rnge)].title()}")
        data = data[:-int(rnge)]
    else: leave("Isches so schwer e positive zahl izgeh, du lappi")
    return data
